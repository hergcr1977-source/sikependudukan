...
export async function POST(request: NextRequest) {
  try {
    ...
    const existing = await db.$queryRawUnsafe<Array<{ id: number }>>(
      `SELECT id FROM "PenerimaSembako" WHERE "nik" = $1`,
      nik
    );
    ...
    const result = await db.$queryRawUnsafe<Array<{ id: number }>>(
      `INSERT INTO "PenerimaSembako" ("noKK", "nik", "namaLengkap", "jenisKelamin", "statusKeluarga", "tanggalLahir", "alamat", "rt", "rw", "keterangan")
  ...
  try {
    ...
    if (deleteAll === 'true') {
      const countResult = await db.$queryRawUnsafe<Array<{ count: bigint }>>(
        `SELECT COUNT(*)::bigint as count FROM "PenerimaSembako"`
      );
    ...
    await db.$executeRawUnsafe(`DELETE FROM "PenerimaSembako" WHERE "id" = $1`, parseInt(id));
